# THU Auto Login —— macOS 实现

> 这是 **macOS** 部分的文档。扩展本身是两端共享的、不含平台代码；
> 跨平台总览见 [`../README.md`](../README.md)，Windows 见 [`../Windows/README.md`](../Windows/README.md)。

macOS 上使用**两个**原生组件：一个无权限的 native messaging host 负责转发，一个菜单栏 App 持有"辅助功能"权限并通过 Quartz 注入按键。原因见第 1、2 节。
...
本文件描述 **macOS** 实现。仓库根目录下的 `THUAutoLogin/` 是两端共享的扩展，其余内容都在 `MacOS/` 里：

```
AutoLogin/
├── THUAutoLogin/               Chrome 扩展（macOS / Windows 共享，零平台代码）
└── MacOS/                      本目录
    ├── shared/
    ...
    │   └── build.sh
    ├── build.sh                一次构建两个组件 → MacOS/build
    ├── install.sh              安装 + 注册（无需 sudo）
    ├── uninstall.sh            卸载
    └── README.md               本文件
```

Windows 实现在 `Windows/`，只需一个原生主机，见 `Windows/README.md`。
